...
<div class="wrapper col3">
  <div id="homecontent">
    <div class="fl_left">
      <div class="column2">
        <h2 style="color:#0000CC">Place FootBall KIT Order !</h2>
        
		
		<center>
		<form id="form1" name="form1" method="post" action="Orderdata.php" >
  ...
  <tr>
	...
	<select name="select" id="select" onchange="MM_jumpMenu('parent',this,0)">
                  ...
                  <?php
						include("config.php");
						$q5=mysql_query("select KitID from Product");
						while($r=mysql_fetch_assoc($q5))
						{
							echo "<option value=Order.php?bid=".$r["KitID"].">".$r["KitID"]."</option>";
						}
						$id=$_REQUEST['bid'];
  ...
  <?php $result = mysql_query("SELECT * FROM  Product where KitID='$id' ");
			$row1= mysql_fetch_array($result);
		  ?>
  <tr>
    <td>Kit ID </td>
    <td><input type="text" name="fid" value="<?php echo $row1['KitID']; ?>" /></td>
  </tr>
  <tr>
  
    <td>Type </td>
    <td><input type="text" name="type" value="<?php echo $row1['Category']; ?>" /></td>
  </tr>
  <tr>
    <td> Name </td>
    <td><input type="text" name="name"  value="<?php echo $row1['KitName']; ?>"  /></td>
  </tr>
  <tr>
    <td>Price </td>
